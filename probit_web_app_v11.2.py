...
def compute_lcx_with_ci(result, x, alpha=ALPHA_LEVEL):
    ...
    if slope <= 0:
        raise ValueError(
            f"LC estimates are not reported because the fitted slope is non-positive ({slope:.4f}). "
...
def interpret_slope_biology(slope, chemical="chemical"):
    ...
    if slope <= 0:
        return (
            f"The fitted slope is non-positive (slope = {slope:.2f}), which is atypical for a standard "
    ...
    if magnitude >= 10:
        ...
        descriptor = "shallow"
    return (
        f"The {chemical} concentration-response is {descriptor} (probit slope = {slope:.2f}). "
...
def pdf_to_bytes(pdf):
    """Return PDF bytes for current fpdf2 and older PyFPDF-compatible return types."""
    output = pdf.output()
...
def create_pdf_single_with_plots(df, df_processed, result, chi_square, df_degrees, p_value,
    ...
    if options.get('include_model_fit', True):
        ...
        pdf.cell(0, 6, f'p-value: {p_value:.4g}', 0, 1)
...
def render_uploaded_dataset_dynamic(uploaded_file, dataset_key):
    ...
    if st.session_state.get(sig_key) != signature:
        ...
        st.session_state[f'{dataset_key}_units_input'] = units_hint
        for field in ('concentration', 'n', 'mortality'):
    ...
    with st.expander(f"📄 {uploaded_file.name}", expanded=True):
        ...
        units = st.text_input(
            "Concentration units (recommended)",
            key=f'{dataset_key}_units_input',
            help="Examples: ppm, %, µg/mL. Comparable units are required for LC50 comparison analysis."
        ).strip()
...
def compare_multi_dose_response_models(analyses):
    ...
    parts = []
    ordered_keys = list(analyses.keys())
...
def build_multi_comparisons(analyses, selected_keys, mode, reference_key=None,
    ...
    for first_key, second_key in pairs:
        ...
        if mode == 'All pairwise':
            ...
            comparison = f'{numerator_name} / {denominator_name}'
...
def create_pdf_multi_report(analyses, global_result, comparisons, comparison_mode,
    ...
    pdf.set_font('Arial', '', 10)
    pdf.cell(0, 6, f'Dataset x log10(concentration) interaction: LR = {global_result["lr_interaction"]:.4f}, df = {global_result["interaction_df"]}, p = {global_result["p_interaction"]:.4g}', 0, 1)
    pdf.cell(0, 6, f'Dataset shift under common slope: LR = {global_result["lr_shift"]:.4f}, df = {global_result["shift_df"]}, p = {global_result["p_shift"]:.4g}', 0, 1)
    ...
    for analysis in analyses.values():
        ...
        values = [
            (name, 52),
            (f'{analysis["slope"]:.4f}', 24),
            (lc50_text, 32),
            (ci_text, 55),
            (f'{analysis["dispersion"]:.3f}', 28),
            (f'{analysis["p_value"]:.4g}', 30),
            (coverage, 35),
        ]
    ...
    if comparisons:
        ...
        for row in comparisons:
            ...
            ps_text = f'{row["p_slope_adjusted"]:.4g}' if np.isfinite(row['p_slope_adjusted']) else 'N/A'
            ph_text = f'{row["p_shift_adjusted"]:.4g}' if np.isfinite(row['p_shift_adjusted']) else 'N/A'
            values = [
...
def build_ai_context():
    ...
    return _json_safe({
        'tool_version': '11.2',
        'individual_analyses': list(individual.values()),
...
def _compatibility_status(records):
    ...
    nonblank_units = {_normalize_column_name(x) for x in units if x}
    return {
...
def _format_comparison_table(comparisons):
    ...
    for row in comparisons:
        rows.append({
            ...
            'Slope p (raw)': row['p_slope_raw'],
            'Slope p (adjusted)': row['p_slope_adjusted'],
            'Shift p (raw)': row['p_shift_raw'],
            'Shift p (adjusted)': row['p_shift_adjusted'],
            'Note': row['note'],
        })
    return pd.DataFrame(rows)
...
def main():
    ...
    st.markdown("**Web Version 11.2** — single- and multi-dataset bioassay probit analysis")
    ...
    records = st.session_state.get('v11_datasets', [])
        ...
        if not valid_records:
            ...
            if st.button("🚀 Run Individual Analysis", type='primary', key='v11_run_individual'):
                try:
                    ...
                    c1.metric("Slope", f"{analysis['slope']:.4f}")
                    c2.metric("Pearson χ²", f"{analysis['chi_square']:.4f}")
                    c3.metric("Pearson p", f"{analysis['p_value']:.4g}")
    ...
    with tabs[2]:
        ...
        if len(valid_records) < 2:
            ...
            selected_records = [record for record in valid_records if record['key'] in selected_keys]
            ...
            if selected_records and compatibility['units_missing']:
                confirmed_units = st.checkbox(
                    "I confirm all selected concentration values are on the same directly comparable scale",
                    value=False,
                    key='v11_confirm_units'
                )
                compatible = compatible and confirmed_units
            ...
            if st.button(
                ...
                if len(analyses) < 2:
                    ...
                    selected_success_keys = [key for key in selected_keys if key in analyses]
                    if comparison_mode == 'Reference vs all' and reference_key not in analyses:
                        ...
                        try:
                            ...
                            with g1:
                                ...
                                st.metric("Slope interaction p", f"{global_result['p_interaction']:.4g}")
                                st.caption(f"df = {global_result['interaction_df']}")
                            with g2:
                                st.metric("Dataset shift LR", f"{global_result['lr_shift']:.4f}")
                                st.metric("Dataset shift p", f"{global_result['p_shift']:.4g}")
...
2. **Individual Analyses** — Select any validated dataset and run the same grouped-binomial probit analysis used in Version 10.7.
...
- Missing chemical/unit metadata require explicit user confirmation before comparison.
